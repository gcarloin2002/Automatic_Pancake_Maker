...
router.post('/', async function create_order(req, res) {
    const { account_id, machine_id, ao_amount, ao_size, ao_status } = req.body; // Destructure necessary fields from request body
    ...
    try {
      ...
      // Step 2: Insert the new order into the account_order table
      const newOrder = await db.one(
        `INSERT INTO account_order (ao_id, account_id, machine_id, ao_amount, ao_size, ao_timestamp, ao_status) 
         ...
         RETURNING *`, 
        [newAoId, account_id, machine_id, ao_amount, ao_size, ao_status] // Use account_id in the insertion
      );
